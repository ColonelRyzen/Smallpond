...
architecture arch_imp of smallpond_axi_v1_0 is
    ...
    signal thalfword_0 : std_logic_vector(15 downto 0) := x"0000"; --temp halfword 0
    signal thalfword_1 : std_logic_vector(15 downto 0) := x"0000"; --temp halfword 1
...
--this code is awful and probably incorrect
	...
	begin
		if m00_axi_aresetn = '0' then --RESET
			...
			step <= "0000";
			sp_over <= '0';
		elsif sp_read='1' and sp_write='0' and step="0000" then --READ
			taddress <= sp_addr;
			m00_axi_araddr <= taddress;
			m00_axi_arvalid <= '1'; --assert read_address_valid
			step <= "0001";
		elsif sp_write='1' and sp_read='0' and step="0000" then --WRITE
			taddress <= sp_addr;
...
--READING PROCESSES--!~
		...
		wait until sp_read='1' and step="0011" and rising_edge(m00_axi_aclk) and m00_axi_rvalid='1';
			if m00_axi_rresp = "00" then
				...
				if sp_op_len = "00" then --already read 2 bytes (discard 2nd). No byte selection in AXI4-lite
					if sp_sign_extend = '1' and m00_axi_rdata(15)='1' then
						sp_data <= x"FFFFFF" & thalfword_0(15 downto 8);
					else
	...
	--process response
	process begin
	   wait until sp_write='1' and step="0100" and rising_edge(m00_axi_aclk);
	...
	--process response
	process begin
       wait until sp_write='1' and step="1000" and rising_edge(m00_axi_aclk);
            if m00_axi_bresp="00" then
                step <= "1111"; --done
                sp_over <= '1';
            else -- "10" transaction failure, "11" incorrect slave address
